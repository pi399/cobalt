...
function Moveable:new(world,mc,mm,mx,my,mxl,myl,mvx,mvy,max,may,sprites)
	mc, mm, mx, my, mxl, myl, mvx, mvy, max, may =
		mc or false, mm or false, mx or 0, my or 0, mxl or 1, myl or 1,
		mvx or 0, mvy or 0, max or 0, may or 0
	...
	if self.moveable then
		...
		for i,ma in ipairs(self.world) do
			if self ~= ma and self.vx and self.vy then
				...
				if xcollision and ycollision then self:correctCollision(ma) end
			end
		end
	end
end
...
function Moveable:correctCollision(ma)
	...
	elseif self.worldycount[ma] < self.worldxcount[ma] then self:correctYCollision(ma) end
end
...
function Moveable:correctXCollision(ma)
	...
	self.y = self.y - overlap ; self.vy = 0 ; self.worldxcount[ma] = 0
end
...
function Moveable:correctYCollision(ma)
	...
	self.x = self.x - overlap ; self.vx = 0 ; self.worldycount[ma] = 0
end
